...
class UploadController extends Controller {
  ...
  async create () {
    ...
    // 只支持上传一个文件
    // 上传文件必须在所有其他的files后面， 否则在拿到的文件流时可能还获取不到fields
    const stream = await ctx.getFileStream()
    // 所有表单字段都能通过 `stream,fields` 获取到
    const filename = path.basename(stream.filename) // 文件名
    const extname = path.extname(stream.filename).toLowerCase() // 文件扩展名
