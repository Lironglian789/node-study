...
class UserService extends Service{
   ...
   * @param {*} payload 
   */
  async uodate(_id, payload) {
    const {ctx, service} = this
    const user = await ctx.service.user.find(_id)
    if(!user) {
      ctx.throw(404, 'user not found')
    }
    return ctx.model.User.findByIdAndUpdate(_is, payload)
  }
  ...
  async index (payload) {
    ...
    if (isPaging) {
      if(search) {
        ...
        .populate('role').skip(skip).limit(Number(pageSize))
        .sort({createAt: -1}).exec()
        count = res.length
      } else {
        res = await this.ctx.model.User.find({})
        .populate('role').sort({createAt: -1}).exec()
